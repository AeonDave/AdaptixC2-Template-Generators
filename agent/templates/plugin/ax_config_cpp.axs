...
function RegisterCommands(listenerType) {
    ...
    var cmd_execute_bof = create_command("execute bof", "Execute Beacon Object File.", "execute bof C:\\tools\\whoami.o", [arg_bof_async, arg_bof_file, arg_bof_args], false);
    ...
    return { commands_windows };
}

function GenerateUI(listeners_type) {

    var arch_list   = create_combobox("arch",   "Architecture", ["x64", "x86"]);
    var format_list = create_combobox("format", "Format",       ["Exe", "Service Exe", "DLL", "Shellcode"]);

    var svc_name = create_line_edit("svc_name", "Service Name", "__NAME_CAP__Service");
    ...
    var container = create_container([
        arch_list, format_list, svc_name,
